...
--
-- Хост: 127.0.0.1:3306
-- Время создания: Апр 29 2026 г., 18:49
...
CREATE TABLE `Activation_Key` (
  `ID_Activation_Key` int NOT NULL,
  `Key_Code` varchar(255) COLLATE utf8mb4_unicode_ci NOT NULL,
  `Status` enum('available','sold','reserved','expired') COLLATE utf8mb4_unicode_ci DEFAULT 'available',
...
CREATE TABLE `Order_Item` (
  ...
  `Activation_Key_ID` int NOT NULL,
...
(9, NULL, '399.99', 'webmoney', 'pending'),
...
ALTER TABLE `Activation_Key`
  ...
  ADD KEY `idx_activation_key_game` (`Game_ID`),
  ADD KEY `idx_activation_key_status` (`Status`);
...
ALTER TABLE `Order_Item`
  ADD PRIMARY KEY (`Order_ID`,`Activation_Key_ID`),
  ADD KEY `Activation_Key_ID` (`Activation_Key_ID`);
...
--
-- AUTO_INCREMENT для таблицы `Activation_Key`
--
ALTER TABLE `Activation_Key`
  MODIFY `ID_Activation_Key` int NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=13;
...
--
ALTER TABLE `Activation_Key`
  ADD CONSTRAINT `activation_key_ibfk_1` FOREIGN KEY (`Game_ID`) REFERENCES `Game` (`ID_Game`) ON DELETE CASCADE ON UPDATE CASCADE;

--
-- Ограничения внешнего ключа таблицы `Order`
--
ALTER TABLE `Order`
  ADD CONSTRAINT `order_ibfk_1` FOREIGN KEY (`Payment_ID`) REFERENCES `Payment` (`ID_Payment`) ON DELETE SET NULL ON UPDATE CASCADE,
  ADD CONSTRAINT `order_ibfk_2` FOREIGN KEY (`Customer_ID`) REFERENCES `Customer` (`ID_Customer`) ON DELETE CASCADE ON UPDATE CASCADE;

--
-- Ограничения внешнего ключа таблицы `Order_Item`
--
ALTER TABLE `Order_Item`
  ADD CONSTRAINT `order_item_ibfk_1` FOREIGN KEY (`Order_ID`) REFERENCES `Order` (`ID_Order`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `order_item_ibfk_2` FOREIGN KEY (`Activation_Key_ID`) REFERENCES `Activation_Key` (`ID_Activation_Key`) ON DELETE CASCADE ON UPDATE CASCADE;
COMMIT;
